...
const CONFIG_MIME_TYPES = ["application/json", "application/json5"];
...
class App {
  ...
  public async download_project() {
    var data = this.project.export_project();
    download(JSON5.stringify(data, null, 4), "project.json5");
  }

  public async download_omnissiah() {
    var data = this.project.export_omnissiah();
    download(JSON5.stringify(data, null, 4), "graphes.json5");
  }
